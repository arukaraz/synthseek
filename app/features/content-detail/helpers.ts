...
export function preloadedTrack(track: MusicTrack): TracklistTrack {
  return {
    ...
    plays: null,
    album: { externalId: track.album.id, name: track.album.name, cover: track.album.images[0]?.url ?? null },
    inLibrary: false,
    requestId: null,
...
export function playlistRequestTracks(tracks: TracklistTrack[]): MusicTrack[] {
  return tracks.map((track) => ({
    ...
    album: {
      id: track.album?.externalId ?? "",
      name: track.album?.name ?? "",
      images: coverImages(track.album?.cover ?? null),
    },
