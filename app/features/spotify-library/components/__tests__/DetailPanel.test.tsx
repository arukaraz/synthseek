...
vi.mock("@hooks/api/queries/spotify/useSpotifyLibraryItemDetail", () => ({
  useSpotifyLibraryItemDetail: () => ({ data: api.detail, isLoading: api.loading }),
}));
...
function Harness({ focusedItem, onBack }: { focusedItem: LibraryItem | null; onBack?: () => void }) {
  ...
  return <DetailPanel focusedItem={stillFocused} draft={draft} onBack={onBack} />;
}
...
describe("the detail pane", () => {
  ...
  it("says it is fetching rather than showing a half-built pane", () => {
    ...
    expect(screen.getByText(enLibrary.spotifyLibrary.detail.loading)).toBeInTheDocument();
  });
  ...
  it("names what is focused and how many tracks it holds", () => {
    ...
    expect(
      screen.getByText(`by arukaraz · ${enLibrary.spotifyLibrary.detail.tracksByline_other.replace("{{count}}", "12")}`)
    ).toBeInTheDocument();
  });
  ...
  it("leaves the owner out of the byline when the source named none", () => {
    ...
    expect(
      screen.getByText(enLibrary.spotifyLibrary.detail.tracksByline_other.replace("{{count}}", "12"))
    ).toBeInTheDocument();
  });
...
describe("what the pane offers", () => {
  ...
  it("goes back the way the caller asked", async () => {
    ...
    await user.click(screen.getByRole("button", { name: enLibrary.spotifyLibrary.detail.back }));
