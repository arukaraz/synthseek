...
describe("SelectionBulkBar", () => {
  ...
  it("exposes the switches with mixed, on and off states", () => {
    ...
    const syncSwitch = screen.getByRole("switch", { name: enLibrary.librarySource.selection.syncAria });
    const importSwitch = screen.getByRole("switch", { name: enLibrary.librarySource.selection.importAria });
    ...
    expect(screen.getByRole("switch", { name: enLibrary.librarySource.selection.syncAria })).toHaveAttribute(
      "aria-checked",
      "false"
    );
  });
  ...
  it("activates sync and import on click", () => {
    ...
    fireEvent.click(screen.getByRole("switch", { name: enLibrary.librarySource.selection.syncAria }));
    fireEvent.click(screen.getByRole("switch", { name: enLibrary.librarySource.selection.importAria }));
  ...
  it("disables the sync switch with a description when no playlists are selected", () => {
    ...
    expect(screen.getByText(enLibrary.librarySource.selection.syncPlaylistsOnly)).toBeInTheDocument();
  });

  it("shows the playlists-only hint for a mixed-type selection", () => {
    renderBar({ hasPlaylists: true, isMixedType: true });
    expect(screen.getByText(enLibrary.librarySource.selection.playlistsOnlyHint)).toBeInTheDocument();
  });

  it("disables every control while a save is in flight", () => {
    renderBar({ disabled: true });
    expect(screen.getByRole("switch", { name: enLibrary.librarySource.selection.importAria })).toBeDisabled();
    expect(screen.getByRole("button", { name: enLibrary.librarySource.selection.clear })).toBeDisabled();
  });
});
