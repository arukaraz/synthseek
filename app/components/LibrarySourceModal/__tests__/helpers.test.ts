...
describe("filterValuesFor", () => {
  it("offers every filter to a source with every item type", () => {
    expect(filterValuesFor(["playlist", "album", "liked"])).toEqual(["all", "playlists", "albums", "liked"]);
  });

  it("offers only all and playlists to a playlist-only source", () => {
    expect(filterValuesFor(["playlist"])).toEqual(["all", "playlists"]);
  });

  it("keeps all even when the source lists no item types", () => {
    expect(filterValuesFor([])).toEqual(["all"]);
  });
});

describe("watchedKeys", () => {
  it("lists both watchers when the source supports both", () => {
    expect(watchedKeys({ playlists: true, savedAlbums: true })).toEqual(["playlists", "savedAlbums"]);
  });

  it("lists only the playlist watcher for a playlist-only source", () => {
    expect(watchedKeys({ playlists: true, savedAlbums: false })).toEqual(["playlists"]);
  });

  it("lists only the saved-albums watcher when that is all the source supports", () => {
    expect(watchedKeys({ playlists: false, savedAlbums: true })).toEqual(["savedAlbums"]);
  });

  it("lists nothing for a source that cannot be watched", () => {
    expect(watchedKeys({ playlists: false, savedAlbums: false })).toEqual([]);
  });
});
...
describe("formatLastSync", () => {
  ...
  it("returns the never label for null", () => {
    expect(formatLastSync(null)).toBe(enLibrary.librarySource.detail.lastSyncNever);
  });

  it("formats a provided date", () => {
    expect(formatLastSync(new Date("2024-01-01T00:00:00Z"))).not.toBe(enLibrary.librarySource.detail.lastSyncNever);
  });
...
describe("libraryTypeLowerLabelKey", () => {
  it("maps each type to its lower label key", () => {
    expect(libraryTypeLowerLabelKey("playlist")).toBe("librarySource.type.playlistLower");
    expect(libraryTypeLowerLabelKey("album")).toBe("librarySource.type.albumLower");
    expect(libraryTypeLowerLabelKey("liked")).toBe("librarySource.type.likedLower");
  });
});
