...
describe("MasterTableRow sync column", () => {
  it("renders the interactive sync chip for a playlist", () => {
    ...
    expect(within(chip).getByText(enLibrary.librarySource.row.enabled)).toBeInTheDocument();
  });
  ...
  it("renders a non-interactive dash for an album with its accessible label", () => {
    ...
    const dash = screen.getByLabelText(enLibrary.librarySource.row.syncUnavailable);
  ...
  it("renders a non-interactive dash for liked songs", () => {
    ...
    expect(screen.getByLabelText(enLibrary.librarySource.row.syncUnavailable)).toBeInTheDocument();
  });
});
