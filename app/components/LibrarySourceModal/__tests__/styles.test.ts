...
describe("connect prompt", () => {
  ...
  it("leaves the icon tint to the provider tone", () => {
    expect(connectPromptIcon()).toContain("rounded-full");
    expect(connectPromptIcon()).not.toContain("text-");
  });
...
describe("chrome bars", () => {
  ...
  it("brandChip carries the foreground and providerTone the provider's brand token", () => {
    expect(brandChip()).toContain("text-fg");
    expect(brandIcon()).not.toContain("text-");
    expect(providerTone({ provider: "spotify" })).toBe("bg-(--brand-spotify)/15 text-(--brand-spotify)");
  });
