...
import { useSaveLibrarySourceChanges } from "@hooks/api/mutations/library-source/useSaveLibrarySourceChanges";
...
import { useLibrarySourceItems } from "@hooks/api/queries/library-source/useLibrarySourceItems";
import { useLibrarySources } from "@hooks/api/queries/library-source/useLibrarySources";
import { useLibrarySubscription } from "@hooks/api/queries/library-source/useLibrarySubscription";
...
import { DEFAULT_IMPORT_CONFIG, PROVIDER_ERROR_CATEGORY } from "./constants";
...
import type { LibraryFilter, LibraryItemType, LibrarySort, LibrarySourceModalProps } from "./types";

export function LibrarySourceModal({ provider, open, onOpenChange }: LibrarySourceModalProps) {
  const { t } = useTranslation("library");
  const sources = useLibrarySources(open);
  const source = sources.data?.find((candidate) => candidate.provider === provider);
  const providerName = source?.name ?? "";
  const connected = source?.connected ?? false;
  const items = useLibrarySourceItems(provider, open && connected);
  const subscription = useLibrarySubscription(provider);
  const save = useSaveLibrarySourceChanges(providerName);
  const invalidateSources = useInvalidateLibrarySources();
  ...
  useEffect(() => {
    ...
    emitFriendlyToast(
      resolveFriendlyError(items.error, {
        category: PROVIDER_ERROR_CATEGORY[provider],
        fallback: {
          title: t("librarySource.loadFailed.title", { provider: providerName }),
          description: t("librarySource.loadFailed.description"),
        },
      })
    );
  ...
  const handleSave = async () => {
    const toImport: Array<{ id: string; type: LibraryItemType; syncEnabled: boolean }> = [];
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogTitle className="sr-only">{t("librarySource.dialog.title", { provider: providerName })}</DialogTitle>
        <DialogDescription className="sr-only">
          {t("librarySource.dialog.description", { provider: providerName })}
        </DialogDescription>
        {source && connected && !reauthNeeded ? (
          <div className={modalGrid()}>
            <ModalTopbar provider={provider} name={providerName} />
            <ModalToolbar
              providerName={providerName}
              itemTypes={source.capabilities.itemTypes}
              watch={source.capabilities.watch}
              filter={filter}
          ...
          <ConnectPrompt source={source} expired={reauthNeeded} />
        )}
      </DialogContent>
    </Dialog>
  );
