...
import { masterControls, masterEmpty, masterScroll, masterXScroll, table } from "../styles";
...
export function MasterTable({ items, isLoading, draft, hiddenOnMobile, selectionBar }: MasterTableProps) {
  ...
  return (
    <div className={masterScroll({ hiddenOnMobile })} onClick={handleBackgroundClick}>
      {selectionBar ? <div className={masterControls()}>{selectionBar}</div> : null}
      {isLoading ? (
        <div className={masterEmpty()}>{t("librarySource.table.loading")}</div>
      ) : items.length === 0 ? (
