...
export function MasterTableRow({
  ...
  const importedLabel = imported ? t("librarySource.row.enabled") : t("librarySource.row.disabled");
    ...
    <tr className={tableRow({ selected, focused })} onClick={onClick} data-master-row-id={item.id}>
      ...
      >
        <span
          ...
          aria-label={selected ? t("librarySource.row.deselect") : t("librarySource.row.select")}
        >
      ...
      <td className={cn(tableCell(), "hidden sm:table-cell")}>
        <span className={typeTag({ tone: libraryTypeTone(item.type) })}>{t(`librarySource.type.${item.type}`)}</span>
      </td>
      ...
      <td className="px-3 py-2 text-center">
        {syncAvailable ? (
          ...
          >
            ...
            {syncEnabled ? t("librarySource.row.enabled") : t("librarySource.row.disabled")}
          </button>
        ) : (
          <span className={syncDash()} role="img" aria-label={t("librarySource.row.syncUnavailable")}>
            -
          </span>
        )}
      </td>
