  ...
  return (
    <div className={detailSection()}>
      <h3 className={detailSectionTitle()}>
        {t("librarySource.metadata.title")} <span className={detailSectionTitleLine()} />
      </h3>
      <div className={metaGrid()}>
        <div className={metaKey()}>{t("librarySource.metadata.sourceId")}</div>
        <div className={metaVal()}>{sourceId}</div>
        {released && (
          <>
            <div className={metaKey()}>{t("librarySource.metadata.released")}</div>
            <div className={metaVal()}>{released}</div>
          </>
        )}
        {label && (
          <>
            <div className={metaKey()}>{t("librarySource.metadata.label")}</div>
            <div className={metaValText()}>{label}</div>
          </>
        )}
        <div className={metaKey()}>{t("librarySource.metadata.lastSync")}</div>
