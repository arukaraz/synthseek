...
**Plex.** Connection status, a reconnect button that signs you in at plex.tv and lets you pick a server, and the server URL that sign-in resolved, shown read-only. Plus three independent behaviour switches:

| Setting             | Default | What it does                                                |
| ------------------- | ------- | ----------------------------------------------------------- |
| Media server rescan | On      | Ask Plex to rescan after an import.                         |
| Playlist sync       | On      | Create and reconcile playlists on Plex.                     |
| Play from Plex      | Off     | Stream a track from Plex when Synthseek has no file for it. |
...
**Media servers.** Navidrome (URL, username, password) and Jellyfin (URL, API key), each with a test button and a **Play from** switch that starts off. Below them, the order Synthseek tries these servers in when it has no file for a track.
...
| Last.fm                   | Similar artists, global charts, and your scrobble history. Its shared secret also lets members send plays to Last.fm.    |
...
Administrators only. Four pages, each holding the list it is about and the one setting that governs it. Every page carries a live count in the sidebar, and the section itself shows a dot while anything is waiting.

**Review**

The files held back because they failed verification, with the reason for each. Nothing arrives until you approve or discard. Its retention is set from the footer of the review queue.

**Duplicates**

Tracks with more than one copy on disk. The ones it could prove identical are already settled; what waits here is what it could not.
...
Alongside it, what the bin currently holds, the files it is keeping, a **Restore** on each one, and a button to empty it.
...
| Setting         | Default                                                         | What it does                                                    |
| --------------- | --------------------------------------------------------------- | --------------------------------------------------------------- |
| Naming template | `{albumartist}/{album}/<Disc {disc:02d}>/{track:02d} - {title}` | The folders and filename every track is given when it is filed. |

Below it, what on disk does not match that template.
...
| ----------------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------- |
| Review queue retention                                | 14 days                   | The footer of the review queue, on the Requests screen or under Maintenance. 1 to 365. |
...
| `MUSIC_LIBRARY_PATH`      | `/music`                     | Your organised library. Read-only is a supported degraded mode, in which imports and reorganising fail. |
...
| Variable                | Default                          | What it is                                                                                                                                                             |
| ----------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ENABLE_LIBRARY_IMPORT` | true                             | Whether to tag and file at all. Replaces `ENABLE_BEETS_IMPORT`, which is still honoured: either one set to false leaves files in the downloads folder.                 |
| `USE_ACOUSTIC_ID`       | true                             | Identify tracks by their audio when tagging.                                                                                                                           |
| `BEETS_COMMAND`         | `beet`                           | Path to the beets executable, used only for the presence check behind the Beets CLI row in Settings.                                                                   |
| `BEETS_CONFIG`          | `/data/config/beets-config.yaml` | Path to a Beets configuration, read only to find the database of an existing Beets library and pick up its MusicBrainz identities. A default is created on first boot. |
| `FPCALC_PATH`           | `fpcalc`                         | Path to the fingerprinting binary.                                                                                                                                     |
| `RSGAIN_PATH`           | `rsgain`                         | Path to the binary that measures how loud each track is, so playback can even out the volume between them.                                                             |
