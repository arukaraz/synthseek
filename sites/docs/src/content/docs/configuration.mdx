...
| Setting         | Default                                                               | What it does                                                    |
| --------------- | --------------------------------------------------------------------- | --------------------------------------------------------------- |
| Naming template | `{albumartist}/{album}/<Disc {disc:02d}>/{track:02d} - {title}.{ext}` | The folders and filename every track is given when it is filed. |

Below it, what on disk does not match that template, and the button that moves it.
...
| `MUSIC_LIBRARY_PATH`      | `/music`                     | Your organised library. Read-only is a supported degraded mode, in which nothing is imported or reorganised. |
