...
| `syncing_plex`       | The playlist it belongs to is being rebuilt on Plex.          |
...
Synthseek tags each verified file and files it into your library. It identifies the track by its audio where it can, falls back to the metadata the catalog gave it where it cannot, so a file is never left untagged.

A track Synthseek still knows the whereabouts of is reported as a duplicate, and only a quality upgrade replaces it. A file Synthseek never filed, or has since lost track of, is not recognised and a second copy is filed.
...
Identifying a track against MusicBrainz happens during the import when the audio can be fingerprinted, and in the background for tracks that carry an ISRC. When it succeeds, Synthseek writes the recording id into the file where it safely can, so the identity travels with your music. A file that already names a recording is left alone.

Imports are strictly serialised. One import runs at a time.

## Step 6, Plex

After a successful import, Plex is asked to rescan the affected part of the library.

If the track belongs to a playlist, that playlist is reconciled on Plex. Reconciliation is incremental: as more tracks of a playlist finish over time, they are added to the remote playlist as they arrive rather than waiting for the whole set.
...
| `timeout`         | A step ran out of the time it was given.                          | Yes                                  |
