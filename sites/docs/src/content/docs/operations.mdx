...
| **Library Scan**           | Indexes the audio files on your library disk, works out which track each unidentified one holds, and notes the ones that are gone. | Every 30 minutes; the disk walk itself twice a day |
...
The same **Jobs** page carries a card for the library scan. It reports how many audio files are on your library disk, how many of them Synthseek recognises as tracks it holds, how many it does not recognise yet, and how many it has stopped finding. Start a scan from the **Library Scan** row above; the card is where you cancel one that is running.
