...
| **Library Scan**           | Indexes the audio files on your library disk and notes the ones that are gone. | Twice a day                         |
| **Library Identification** | Works out which track each unidentified file holds, so it joins your library.  | Every 30 minutes, in small batches  |
...
The same **Jobs** page carries a card for the library scan. It reports how many audio files are on your library disk, how many of them Synthseek recognises as tracks it holds, how many it does not recognise yet, and how many it has stopped finding. You can start a scan there and cancel one that is running.
